      ...
      <div className="space-y-12">
        {POSTS.map((post) => (
          <article key={post.id} className="glass-panel p-10 rounded-3xl group transition-all duration-500 hover:-translate-y-1 hover:border-emerald-500/30 hover:shadow-[0_0_40px_rgba(16,185,129,0.1)]">
            ...
            <div>
              <Link href={post.slug} className="text-sm font-bold tracking-wider text-emerald-500 hover:text-emerald-400 uppercase transition-colors flex items-center gap-2">
                Read Dispatch <span className="group-hover:translate-x-1 transition-transform">→</span>
              </Link>
            </div>
          </article>
        ))}
